//////////////////////////////////////////////////////////////////////////////////////
// sources can be found at the bottom og the code
...
app.post("/api/messages", requireLogin, async (req, res) => { // api for messages
  try {
    ...
    const message = String(req.body.message || "").trim(); // setting up consts

    if (!recipientId || !message) { // validation check
      return res.status(400).json({
        error: "Recipient and message are required",
      });
    }

    const expertResult = await pool.query( // messages sent to expert
      `SELECT id, first_name, last_name, email
       FROM users
       WHERE id = $1`,
      [recipientId]
    ...
    if (expertResult.rows.length === 0) { //validation check
      return res.status(404).json({
        error: "Expert not found",
      });
    }

    const senderResult = await pool.query( // getting sender details
    ...
    const expert = expertResult.rows[0]; // getting sender and reciever details assigning to consts
    const sender = senderResult.rows[0];

    await pool.query( // query into the neon database messages
    ...
    await transporter.sendMail({ // email details sent to expert 
      ...
      subject: `New STEP message from ${sender.first_name} ${sender.last_name}`,
      text: `
Hi ${expert.first_name},

You have received a new message on STEP.
...
${message}
      `.trim(),
      html: `
        <h2>New STEP message</h2>
        <p>Hi ${expert.first_name},</p>
        <p>You have received a new message on STEP.</p>
...
app.post("/api/contact-support", async (req, res) => { // post for contacting support/reporting people
  try {
    const email = String(req.body.email || "").trim().toLowerCase(); // setting up consts from the form
    const reason = String(req.body.reason || "").trim();
    const summary = String(req.body.summary || "").trim();
    const description = String(req.body.description || "").trim();

    if (!email || !reason || !summary || !description) { // val check to ensure all form boxes have been filled in
      return res.status(400).json({
        error: "All fields are required",
      });
    }

    await transporter.sendMail({ // setting up actual email
      from: `"STEP Support Form" <${process.env.EMAIL_USER}>`, // makes sure inputted email is used instead of the envirmoental one in render
      to: process.env.EMAIL_USER,
      replyTo: email,
      subject: `STEP Support: ${summary}`, // uses summary from form
      text: `
New support message
From: ${email}
Reason: ${reason}
...
${description}
      `.trim(),
      html: `
        <h2>New STEP support message</h2>
    ...
    res.json({
      message: "Support message sent!", // json checking message sent
    });
  } catch (error) {
...
///////////////////////////////////////////////////////////////////////////////////////////////
// https://neon.com/docs/guides/render 
// https://neon.com/guides/nodejs-queue-system
